...
Highly compatible with GNU `tr` for translate, delete, squeeze, and complement. Output is byte-identical to GNU `tr` for all of these (verified in the `yupsh/bin-tr` Docker integration harness against Debian coreutils), with one line-orientation divergence on the input's trailing newline.
